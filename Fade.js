...
function Alpha (object : Object, start : float, end : float, timer : float, easeType : EaseType) {
    ...
    while (t < 1.0) {
        ...
        if (objectType == GUITexture)
            RenderSettings.fogColor.a = Mathf.Lerp(start, end, Ease(t, easeType)) * .5;
        else
            RenderSettings.fogColor.a = Mathf.Lerp(start, end, Ease(t, easeType));
        yield;
    }
}
...
function Colors (object : Object, start : Color, end : Color, timer : float, easeType : EaseType) {
    ...
    while (t < 1.0) {
        t += Time.deltaTime * (1.0/timer);
        if (objectType == GUITexture)
            RenderSettings.fogColor = Color.Lerp(start, end, Ease(t, easeType)) * .5;
        else
            RenderSettings.fogColor = Color.Lerp(start, end, Ease(t, easeType));
        yield;
    }
}
...
function Colors (object : Object, colorRange : Color[], timer : float, repeat : boolean) {
    ...
    while (true) {
        ...
        while (t < 1.0) {
            t += Time.deltaTime * (1.0/timer);
            if (objectType == GUITexture)
               	RenderSettings.fogColor = Color.Lerp(colorRange[i], colorRange[(i+1) % colorRange.Length], t) * .5;
            else
                RenderSettings.fogColor = Color.Lerp(colorRange[i], colorRange[(i+1) % colorRange.Length], t);
            yield;
        }
